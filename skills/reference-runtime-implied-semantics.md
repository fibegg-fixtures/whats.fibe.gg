---
name: reference-runtime-implied-semantics
...
# Reference: template signals that imply runtime behavior
...
- Compose `ports:` must be absent only when `x-fibe.gg.metadata.preserve_ports: true` is set
...
- Service-level `ports:` lines are removed from compiled output by default. Add them for local `docker compose up` if useful; they survive only when `x-fibe.gg.metadata.preserve_ports: true`.
- In job-mode templates, the routing labels (`fibe.gg/port`, `fibe.gg/visibility`, `fibe.gg/subdomain`) and any `traefik.*` labels are removed from the generated compose; other labels pass through unchanged.
