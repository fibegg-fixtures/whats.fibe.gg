...
# Decide: static vs dynamic service
...
## Source-only dynamic services

Some templates use services only to make multiple source repositories available to a controller service. These services do not run application code and should normally exit successfully after Fibe has cloned/mounted the source.

Use a small static runner image and omit `build:`:
...
Do not add `build:` to this pattern just to make it "dynamic". `fibe.gg/repo_url` already makes it dynamic; `build:` changes the workflow to build and can make Fibe try to build a repository that is only meant to be mounted.
