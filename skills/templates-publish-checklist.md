---
name: templates-publish-checklist
...
- [ ] Publicly expose unauthenticated admin consoles (pgAdmin, MinIO Console, Sidekiq dashboard) without `fibe.gg/visibility: internal` (which adds Basic Auth).
...
- `fibe.gg/visibility: External` (must be lowercase `internal` / `external`), or the removed combined expose label (rejected as unknown).
