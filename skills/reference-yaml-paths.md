...
# Reference: YAML path syntax for `path` / `paths` bindings
...
You may also write `services.web.labels.fibe\.gg/subdomain` for clarity; both work.
...
x-fibe.gg:
  variables:
    ...
    BAD_INDEX:
      path: services.web.environment.[0]             # ❌ wrong shape: `environment` is a map — this writes a literal "0" key, not an index
```
