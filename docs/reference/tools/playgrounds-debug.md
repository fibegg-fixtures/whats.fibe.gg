...
slug: /reference/tools/playgrounds-debug
sidebar_label: "Playgrounds Debug"
image: /img/og/reference-tools-playgrounds-debug.png
keywords: ["Fibe", "Tool", "fibe", "tool", "playgrounds", "debug"]
...
2. Fibe returns `request_id` + `status_url` for refreshed diagnostics.
3. The SDK polls `GET /api/playgrounds/:id/debug/:request_id` until terminal.
4. Returns the final payload.

Cached read (`refresh:false`) skips the job and computes diagnostics synchronously.
