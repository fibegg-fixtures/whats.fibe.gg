...
- **Raw Compose `ports:` are removed from compiled compose by default**
  Local `ports:` can remain for `docker compose up`, but Fibe strips them before launch unless `x-fibe.gg.metadata.preserve_ports: true`.
...
You wrote a label under `fibe.gg/` that isn't in the whitelist. The 20 supported labels are listed in [reference-fibe-labels](reference-fibe-labels.md).
...
This appears only when raw Compose ports are being preserved. Host-port pinning is incompatible with rolling updates.

**Fix:** Remove `ports:` or leave `x-fibe.gg.metadata.preserve_ports` unset/false. The service is reachable via `fibe.gg/port`. See [recipe-ports-to-expose](recipe-ports-to-expose.md), [recipe-strip-incompatible-keys](recipe-strip-incompatible-keys.md).
...
You set `fibe.gg/visibility` on a service that has no `fibe.gg/port`.
...
An invalid cron expression is rejected when the schedule is saved or imported (error: `invalid schedule format`), so a stored schedule's cron is already valid.
