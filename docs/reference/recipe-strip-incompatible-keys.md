...
slug: /reference/recipe-strip-incompatible-keys
sidebar_label: "Strip Incompatible Keys"
image: /img/og/reference-recipe-strip-incompatible-keys.png
keywords: ["Fibe", "Recipe", "recipe", "strip", "incompatible", "keys"]
...
### Route with `fibe.gg/port`, not raw `ports:`

Rewrite user-facing HTTP exposure to `fibe.gg/port`; that is the Fibe routing contract. Raw Compose `ports:` can remain for local `docker compose up`, and Fibe strips them by default during launch generation. Only `x-fibe.gg.metadata.preserve_ports: true` keeps raw host bindings on Fibe; use that rarely because it makes templates less portable and brings back host-port conflict validation. See [recipe-ports-to-expose](recipe-ports-to-expose.md).
