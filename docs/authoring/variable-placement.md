...
Write the array index as its own dot-separated segment: `services.web.environment.[0]` and `services.web.command.[2]` (or plain `.0` / `.2`). A bracket attached to the key name, like `command[2]`, is treated as a literal key named `command[2]`, not an index. Dotted keys like `fibe.gg/port` are matched as **single segments** under `labels:` even though they contain dots — Fibe knows the difference.
