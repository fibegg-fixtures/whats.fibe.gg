...
Launching from a template requires a funded target Marquee. Unpaid Marquees fail with `MARQUEE_NOT_FUNDED`.

Spins up a Playground from an Import Template version. Skips repo creation and calls `POST /api/import_templates/:id/launches`.
...
2. Fibe resolves the template version (latest if unspecified), compiles the Compose YAML with provided variables, applies overrides.
